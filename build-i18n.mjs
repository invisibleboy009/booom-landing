...
const slBlock = self => {
  const cols = SL_GROUPS.map(([title, items]) => {
  ...
  return `<!-- site-links:start (generated by build-i18n.mjs) -->\n<aside class="sl" aria-label="Ďalšie nástroje a sprievodcovia" data-slot="site-links"><style>${SL_CSS}</style><p class="sl-h">Ďalšie nástroje a sprievodcovia</p><div class="sl-grid">${cols}</div></aside>\n<!-- site-links:end -->\n`
}
...
for (const [route, file] of slPages) {
  ...
  const out = clean.slice(0, at) + slBlock(route) + clean.slice(at)
